// AUTO-GENERATED — edit templates/planner.html, not this file
document.getElementById('tab-planner').innerHTML = `<div class="planner-layout">
    ...
    <div class="planner-sidebar">
      ...
      <div class="sidebar-section">
        ...
        <div id="planner-weather" class="planner-weather-bar" style="display:none"></div>
      </div>

      <div class="sidebar-section">
        <div class="d-flex justify-content-between align-items-center mb-1">
          <div class="sidebar-section-title mb-0">Saved Locations</div>
          <button id="save-location-btn" class="btn btn-sm btn-outline-success py-0 px-2"
                  title="Set a location first" style="font-size:0.72rem" disabled>＋ Save</button>
        </div>
      ...
      <!-- Time slider removed from sidebar — merged into bottom map bar -->
      ...
      <div class="sidebar-section">
        ...
        <div class="d-flex gap-1 flex-wrap mb-1">
          ...
          <button class="btn btn-sm map-layer-btn" data-layer="terrain">🏔 Topo</button>
        </div>
        <div class="d-flex gap-1 flex-wrap">
          <button class="btn btn-sm map-layer-btn map-layer-btn--free" data-layer="topplusopen" title="Free — no API key required">🗾 TopPlus</button>
          <button class="btn btn-sm map-layer-btn map-layer-btn--tf" data-layer="tf-landscape" title="Thunderforest Landscape (free API key required)">🌲 Landscape</button>
          <button class="btn btn-sm map-layer-btn map-layer-btn--tf" data-layer="tf-outdoors" title="Thunderforest Outdoors (free API key required)">⛰ Outdoors</button>
          <button class="btn btn-sm map-layer-btn map-layer-btn--free" data-layer="light-pollution" title="NASA VIIRS Night Lights — light pollution map (free)">🌃 Light Poll.</button>
        </div>
        <div class="tf-key-hint" id="tf-key-hint">🔑 Thunderforest layers need a free API key at <a href="https://www.thunderforest.com/docs/apikeys/" target="_blank" rel="noopener">thunderforest.com</a></div>
      </div>
      ...
      <div class="sidebar-section">
        ...
        <div class="d-flex flex-column gap-1">
          ...
          <div class="form-check form-switch mb-0">
            <input class="form-check-input" type="checkbox" id="show-shadow" />
            <label class="form-check-label small" for="show-shadow">🌑 Shadow Dir.</label>
          </div>
          <div class="form-check form-switch mb-0">
            <input class="form-check-input" type="checkbox" id="show-cardinal" />
            <label class="form-check-label small" for="show-cardinal">🧭 Cardinal Lines</label>
          </div>
          <div class="form-check form-switch mb-0">
            <input class="form-check-input" type="checkbox" id="show-rings" />
            <label class="form-check-label small" for="show-rings">📏 Distance Rings</label>
          </div>
        </div>
      </div>

      <div class="sidebar-section">
        <div class="sidebar-section-title">Camera FOV</div>
        <p class="text-secondary mb-2" style="font-size:0.7rem">Visualize lens field of view on map</p>
        <div class="d-flex gap-1 mb-1">
          <select id="fov-sensor" class="form-select form-select-sm" style="flex:1.2">
            <option value="ff">Full Frame</option>
            <option value="apsc">APS-C</option>
            <option value="mft">Micro 4/3</option>
            <option value="one">1-inch</option>
            <option value="phone">Phone</option>
          </select>
          <div class="input-group input-group-sm" style="flex:1">
            <input id="fov-focal" type="number" class="form-control form-control-sm" value="50" min="8" max="800" />
            <span class="input-group-text" style="font-size:0.7rem">mm</span>
          </div>
        </div>
        <div class="d-flex gap-1 align-items-center mb-1">
          <label class="text-secondary" style="font-size:0.7rem;white-space:nowrap">Bearing</label>
          <input id="fov-bearing" type="number" class="form-control form-control-sm" value="0" min="0" max="360" style="flex:1" />
          <span class="text-secondary" style="font-size:0.7rem">°N</span>
        </div>
        <div class="form-check form-switch mb-0">
          <input class="form-check-input" type="checkbox" id="show-fov" />
          <label class="form-check-label small" for="show-fov">📷 Show FOV Cone</label>
        </div>
      </div>
      ...
      <button id="sky-view-btn" class="btn btn-sm w-100 sky-btn mb-1">🌐 Open Sky View</button>
      <button id="ar-view-btn" class="btn btn-sm w-100 ar-btn mb-2">📡 Live AR View</button>
    ...
    <div class="map-wrap">
      ...
      <!-- ── Altitude Timeline (collapsible panel above time bar) ──────────── -->
      <div id="timeline-overlay" class="timeline-overlay collapsed">
        ...
        <div id="tl-events-bar" class="tl-events-bar"></div>
      ...
      <!-- ── Bottom Time Bar (always visible, sky-gradient slider) ─────────── -->
      <div id="map-time-bar" class="map-time-bar">
        <div class="map-time-bar-inner">
          <div class="map-time-bar-header">
            <!-- Mobile FAB lives here so it's always in the time bar, never behind the timeline -->
            <button id="mobile-sidebar-fab" class="mobile-map-fab" aria-label="Open controls">⚙ Controls</button>
            <span class="map-time-bar-label" id="map-time-bar-label">6:00 AM</span>
            <span class="map-time-bar-status" id="map-time-bar-status"></span>
            <span class="map-time-bar-hint">← drag to change time →</span>
          </div>
          <input type="range" id="plan-time-slider" class="map-time-bar-slider"
                 min="0" max="1439" value="360" step="5" />
          <div class="map-time-bar-ticks">
